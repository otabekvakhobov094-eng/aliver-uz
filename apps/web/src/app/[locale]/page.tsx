  ...
  return <>
    ...
    <main className={styles.main}>
      <section className={styles.hero}>
        {hero?.imageUrl ? <div className={styles.heroImage} style={{ backgroundImage: `url(${hero.imageUrl})` }} /> : null}
        <div className={`${styles.shell} ${styles.heroGrid}`}>
          <div className={styles.heroCopy}>
            ...
            <div className={styles.actions}>
              <Link className={styles.primaryAction} href={hero?.ctaUrl ?? `/${locale}/katalog`}>{(locale === 'ru' ? hero?.ctaLabelRu : hero?.ctaLabelUz) ?? (locale === 'ru' ? 'Смотреть каталог' : 'Katalogni ko‘rish')} <span>↗</span></Link>
          ...
          {!hero?.imageUrl ? <div className={styles.heroVisual} aria-label="ALIVER collection"><div className={styles.glow} /><div className={`${styles.productShape} ${styles.productShapeTall}`}><span>ALIVER</span><small>BEAUTY</small></div><div className={`${styles.productShape} ${styles.productShapeRound}`}><span>ALIVER</span><small>GLOW</small></div><div className={styles.visualCaption}><strong>New ritual</strong><span>{locale === 'ru' ? 'Коллекция 2026' : '2026 kolleksiyasi'}</span></div></div> : null}
        </div>
      </section>

      <section className={`${styles.shell} ${styles.promises}`}>
      ...
      <section className={`${styles.shell} ${styles.story}`}><div className={styles.storyArt}><span>ALIVER</span></div><div className={styles.storyCopy}><p className={styles.eyebrow}>{locale === 'ru' ? 'Философия ALIVER' : 'ALIVER falsafasi'}</p><h2>{locale === 'ru' ? 'Уход, созданный для вашей уверенности' : 'O‘zingizga bo‘lgan ishonch uchun yaratilgan parvarish'}</h2><p>{locale === 'ru' ? 'Красота — это ежедневное внимание к себе. Эффективные формулы, приятные текстуры и современный дизайн.' : 'Go‘zallik o‘zingizga har kuni e’tibor berishdan boshlanadi. Samarali formulalar, yoqimli teksturalar va zamonaviy dizayn.'}</p><Link className={styles.textAction} href={`/${locale}/sahifa/biz-haqimiz`}>{locale === 'ru' ? 'Узнать больше' : 'Batafsil bilish'} <span>→</span></Link></div></section>
      ...
      <section className={`${styles.shell} ${styles.newsletter}`}><div><p className={styles.eyebrow}>ALIVER CLUB</p><h2>{locale === 'ru' ? 'Будьте ближе к миру красоты' : 'Go‘zallik olamiga yanada yaqin bo‘ling'}</h2></div><Link className={styles.lightAction} href={`/${locale}/hamkorlik`}>{locale === 'ru' ? 'Присоединиться' : 'Hamjamiyatga qo‘shilish'} <span>→</span></Link></section>
    </main>
...
function ProductRow({ title, kicker, href, items, locale }: { title: string; kicker: string; href: string; items: Awaited<ReturnType<typeof catalogApi.products>>['items']; locale: 'uz' | 'ru' }) { if (!items.length) return null; return <section className={`${styles.shell} ${styles.section}`}><Heading kicker={kicker} title={title} href={href} link={locale === 'ru' ? 'Смотреть все' : 'Hammasini ko‘rish'} /><div className="alv-grid">{items.map((product) => <ProductCardView key={product.id} product={product} locale={locale} />)}</div></section>; }
